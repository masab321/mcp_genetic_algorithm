...
// knobs
int pop_size = 50;
int regen_population_limit = 50;
int generations = 200;
int naive_restart_number = 100;
int number_of_mutations = 2;
int elite_n = 1;
...
void expand_clique(vector<int>& vertices) {
    vector<int> best_clique = naive_clique(vertices);
...
vector<int> naive_clique(vector<int>& chromosome) {
    ...
    while (restart_number > 0) {
        ...
        restart_number--;
        start_point = random_number(0, N - 1);
    }
    return best_clique;
}
